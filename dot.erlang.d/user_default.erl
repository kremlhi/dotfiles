...
tc(M,F,A) ->
    element(1,timer:tc(M,F,A))/1.0e6.

to_src(Beam) ->
    {ok,{_,[{abstract_code,{_,AC}}]}} = beam_lib:chunks(Beam,[abstract_code]),
    io:format("~s~n", [erl_prettypr:format(erl_syntax:form_list(AC))]).

%% The gen___tcp and ssl sockets have a {packet, Type} socket option to
%% aid in decoding a number of protocols. The function
%% erlang:decode_packet/3 has a good description on what the various Type
%% values can be and what they do.

%% Together with a {active, once} or {active, true} setting, each framed
%% value will be delivered as a single message.

%% Examples: the packet http mode is used heavily for iserve and the
%% packet fcgi mode for ifastcgi. I can imagine that many of the other
%% http servers use packet http as well.


h2b(B) ->
    << <<(list_to_integer([X,Y], 16))>> || <<X,Y>> <= B >>.

b2h(B) ->
    << <<(hd(integer_to_list(X, 16)))>> || <<X:4>> <= B >>.
